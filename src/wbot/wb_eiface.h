...
#pragma pack()
	...
	public:
		...
		void set_actor_origin(AActor* actor, int x, int y, uint32_t angle, bool teleportFx); // angle is given in degrees

		//
		// math funcs
		//
		void MakeVectors(uint32_t angle, vec3& forward, vec3& right);
		int PointToAngle2(float x1, float y1, float x2, float y2); // todo: convert vector angles to yaw/pitch
		...
		// map queries
		//
		const char* map_name(); // lump name of the current map
		bool intermission_active();
		int tics(); // tics since level start (1 second = 35 tics)
		...
		std::vector<MapLine*> get_crossed_lines(const vec2& pos, int radius); // get lines that the given box intersects
		...
		std::vector<TraceIsect> TraceIntersections(vec2 start, vec2 end); // returns all walls/sectors intersected by the given line
		bool TraceImpassable(vec2 start, vec2 end); // returns true if the trace intersects any impassable walls
		bool TraceSectorEdge(vec2 start, vec2 end, vec2& edge, wbot::MapLine** line); // trace until the first intersected sector edge
		...
		void printf(const char* fmt, ...); // print a message to the default output stream
		void print_hud_text(const char* msg, float x, float y, uint32_t id); // ID must be unique per hud element
