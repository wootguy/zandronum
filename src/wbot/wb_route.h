...
public:
	...
	float m_lastElevZ = 0;			// last height of the elevator we've been standing on
	...
	vec2 jumpBackupPos = vec2(0, 0);		// position to move to for a running start
	vec2 jumpStartPos = vec2(0,0);			// position where the jump begins
	vec2 jumpEndPos = vec2(0,0);			// position where the jump ends
